...
<C:addressbook-query xmlns:D="DAV:" xmlns:C="urn:ietf:params:xml:ns:carddav">
  ...
  try {
    ...
    logger.debug('CardDAV Response:', response.status, response.statusText);

    if (!response.ok) {
      const errorBody = await response.text();
      logger.error('CardDAV request failed:', response.status, response.statusText);
      logger.error('Error body:', errorBody.substring(0, 500));
      return null;
    }
    ...
    for (const resp of responseArray) {
      ...
      try {
        ...
        logger.error('Error parsing vCard:', parseError);
        continue;
      }
    }
    ...
    logger.error('CardDAV search error:', error);
    return null;
  }
}
...
async function searchCardDAVByName(name) {
  ...
  try {
    ...
    logger.error('CardDAV name search error:', error);
    return null;
  }
}
...
async function searchCardDAVByPhone(phone) {
  ...
  try {
    ...
    logger.error('CardDAV phone search error:', error);
    return null;
  }
}
 ...
 * @param {string} uidOrHref - Contact UID or full href path
 * @param {string} birthdate - Birthdate to set
 * @param {string} href - Optional: Full CardDAV href path (preferred over UID)
 */
async function updateCardDAVContactBirthdate(uidOrHref, birthdate, href = null) {
  const config = getCardDAVConfig();

  if (!config.enabled || !uidOrHref || !birthdate) {
    logger.error('[CardDAV] Cannot update birthdate: missing parameters', {
  ...
  try {
    ...
    // Determine the contact URL
    let contactUrl;
    if (href) {
      // If href is provided, use it (it may be a relative or absolute URL)
      if (href.startsWith('http')) {
        contactUrl = href;
      } else {
        // Relative href - construct full URL
        const baseUrl = config.url.replace(/\/$/, '');
        contactUrl = href.startsWith('/') ? `${baseUrl}${href}` : `${baseUrl}/${href}`;
      }
    } else {
      // Fall back to old behavior: construct URL from UID
      contactUrl = config.url.replace(/\/$/, '') + `/${uidOrHref}.vcf`;
    }

    logger.debug('[CardDAV] Updating birthdate for contact at:', contactUrl);
    ...
    if (!getResponse.ok) {
      logger.error(`[CardDAV] Failed to fetch contact for update: ${getResponse.status} ${getResponse.statusText}`);
      return false;
    }
    ...
    if (putResponse.ok || putResponse.status === 204) {
      logger.info(`[CardDAV] Successfully updated birthdate to ${formattedBirthdate}`);
      return true;
    } else {
      logger.error(`[CardDAV] Failed to update contact: ${putResponse.status} ${putResponse.statusText}`);
      return false;
    }
  } catch (error) {
    logger.error('[CardDAV] Update birthdate error:', error);
    return false;
  }
}
...
async function createCardDAVContact(name, phone, birthdate) {
  ...
  try {
    ...
    logger.error('CardDAV create contact error:', error);
    return false;
  }
}
...
END:VCARD`;
  ...
  try {
    ...
    if (response.ok || response.status === 201) {
      ...
      logger.error('Failed to create contact:', response.status, response.statusText);
      return false;
    }
  } catch (error) {
    logger.error('CardDAV create contact error:', error);
  ...
  try {
    ...
    for (const resp of responseArray) {
      ...
      try {
        ...
        logger.error('Error parsing vCard for photo:', parseError);
        continue;
      }
    }
    ...
    logger.error('CardDAV search with photo error:', error);
    return null;
  }
}
...
async function getContactByNameWithDetails(name) {
  ...
  try {
    ...
    logger.error('CardDAV name search with details error:', error);
    return null;
  }
}
...
async function getContactByPhoneWithDetails(phone) {
  ...
  try {
    ...
    logger.error('CardDAV phone search with details error:', error);
    return null;
  }
}
...
async function fuzzySearchCardDAVByName(searchName, minSimilarity = 0.5) {
  ...
  try {
    ...
    logger.error('CardDAV fuzzy search error:', error);
    return [];
  }
}
