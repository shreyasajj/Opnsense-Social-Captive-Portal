...
async function reconfigureCaptivePortal(config) {
  ...
  logger.info('Captive portal reconfigured successfully');
  return true;
}
...
async function allowMacInOPNsense(mac) {
  ...
  if (!config.enabled) {
    logger.info('OPNsense API not configured - skipping MAC whitelist');
  ...
  logger.debug(`Adding MAC ${normalizedMac} to OPNsense whitelist...`);

  return opnsenseLock.withLock(async () => {
    // Step 1: Get zone UUID
    logger.debug('Step 1: Getting zone UUID...');
    const zoneUUID = await getZoneUUID(config);
    logger.debug(`Found zone UUID: ${zoneUUID}`);

    // Step 2: Get current zone config
    logger.debug('Step 2: Getting zone config...');
    ...
    logger.debug('Step 7: Reconfiguring captive portal...');
    await reconfigureCaptivePortal(config);

    logger.info(`Successfully added MAC ${normalizedMac} to whitelist`);
    return { success: true };
  });
}
...
async function revokeMacInOPNsense(mac) {
  ...
  if (!config.enabled) {
    logger.info('OPNsense API not configured - skipping MAC removal');
    return { success: true, skipped: true };
  }
  ...
  logger.debug(`Removing MAC ${normalizedMac} from OPNsense whitelist...`);
  ...
  return opnsenseLock.withLock(async () => {
    ...
    if (!currentMacs.includes(normalizedMac)) {
      logger.info(`MAC ${normalizedMac} not in whitelist`);
    ...
    logger.info(`Successfully removed MAC ${normalizedMac} from whitelist`);
    return { success: true };
  });
}
...
async function getArpTable() {
  ...
  try {
    ...
    if (!response.ok) {
      logger.error('Failed to get ARP table:', response.status);
      return [];
    }
