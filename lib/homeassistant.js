...
async function notifyHomeAssistant(event, data) {
  ...
  if (!config.enabled) {
    logger.info('Home Assistant not configured');
    return false;
  }
  ...
  try {
    ...
    logger.error('Home Assistant notification error:', error);
    return false;
  }
}
...
async function registerDeviceTracker(mac, name, isPhone) {
  ...
  try {
    ...
    logger.error('Device tracker registration error:', error);
    return false;
  }
}
...
async function callService(domain, service, data = {}) {
  ...
  try {
    ...
    logger.error('Home Assistant service call error:', error);
    return false;
  }
}
...
async function getState(entityId) {
  ...
  try {
    ...
    logger.error('Home Assistant get state error:', error);
    return null;
  }
}
